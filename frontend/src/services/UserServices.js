...
export const deleteLike1 = async (id) => {
  const  data  = await axios.delete(`http://localhost:5000/api/v1/hall/likes/${id}` , config);
  return data;
};
export const addLike1 = async (id) => {
  const { data } = await axios.patch(`http://localhost:5000/api/v1/hall/likes/${id}` ,{}, config);
  return data;
};

export const deleteLike = async (id) => {
  const  data  = await axios.delete(`http://localhost:5000/api/v1/books/likes/${id}` , config);
  return data;
};
export const addLike = async (id) => {
  const { data } = await axios.patch(`http://localhost:5000/api/v1/books/likes/${id}` ,{}, config);
  return data;
};
